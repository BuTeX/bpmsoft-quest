...
assert(elements.get("chapter4-next-action").textContent.startsWith("Шаг 1 из 5"), "Chapter 4 did not explain the initial image-audit step");
firstStage.hotspots.forEach((spot) => api.inspectHotspot(spot.id));
assert(elements.get("chapter4-next-action").textContent.startsWith("Шаг 2 из 5"), "Chapter 4 did not advance the guide to cause selection");
...
assert(elements.get("chapter4-next-action").textContent.startsWith("Шаг 3 из 5"), "Chapter 4 did not advance the guide to mechanism selection");
assert(api.assignCard("mechanism", firstStage.solution.mechanism), "Correct mechanism card could not be selected");
assert(elements.get("chapter4-next-action").textContent.startsWith("Шаг 4 из 5"), "Chapter 4 did not advance the guide to verification selection");
assert(api.assignCard("test", firstStage.solution.test), "Correct test card could not be selected");
assert(elements.get("chapter4-next-action").textContent.startsWith("Шаг 5 из 5"), "Chapter 4 did not advance the guide to chain confirmation");
assert(!api.checkChain(), "Invalid cause-mechanism-test chain was accepted");
assert(api.getState().energy === 3, "Invalid Chapter 4 chain did not consume one energy cell");
